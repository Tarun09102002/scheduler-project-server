...
exports.login_user = async (req, res) => {
	...
	if (!user) {
		...
		if (validPassword) {
			...
			return res.status(200).send({ message: "successful", token: token });
		} else {
			console.log("invalid");
			return res.status(401).send({ message: "unsuccessful" });
		}
...
exports.delete_event = async (req, res) => {
	...
	if (event) {
		...
		const user = await User.findByIdAndUpdate(event.user, {
			$pull: { events: event._id },
		}).catch((err) => console.log(err));
	} else {
		const meet = await Meet.findById(id).catch((err) => console.log(err));
		if (meet) {
			const user = await User.findByIdAndUpdate(meet.createdby, {
				$pull: { meets: meet._id },
			}).catch((err) => console.log(err));
			for (let i = 0; i < meet.participants.length; i++) {
				const notification = await Notification.findOneAndDelete({
					receiver: meet.participants[i],
				}).catch((err) => console.log(err));
				const user = await User.findByIdAndUpdate(meet.participants[i], {
					$pull: {
						meets: meet._id,
						pendingMeets: meet._id,
...
exports.edit_event = async (req, res) => {
	...
	if (event) {
		await Event.findByIdAndUpdate(id, req.body).catch((err) =>
			console.log(err)
		);
	} else {
		const meet = await Meet.findById(id).catch((err) => console.log(err));
		if (meet) {
			await Meet.findByIdAndUpdate(id, req.body).catch((err) =>
				console.log(err)
			);
		} else {
			res.send("Event not found");
...
exports.add_event = async (req, res) => {
	...
	console.log(req.body);
	const event = await Event.create({ ...req.body, user: userId }).catch((err) =>
		console.log(err)
	);
	// console.log(event + " " + 191);
	const user = await User.findById(userId).catch((err) => console.log(err));
...
exports.add_meet = async (req, res) => {
	...
	if (unavailableUsers.length > 0) {
		...
		const meet = await Meet.create({
			...req.body,
			createdby: userId,
			participants: Array.from(availableUsers),
		}).catch((err) => console.log(err));
		const user = await User.findById(userId).catch((err) => console.log(err));
		...
		availableUsers.forEach(async (participant) => {
			const userParticipant = await User.findById(participant).catch((err) =>
				console.log(err)
			);
			...
			const notification = await Notification.create({
				...
				type: "meet",
				meet: meet.id,
				message: `${user.name} has invited you to a meet`,
			}).catch((err) => console.log(err));
...
exports.fetch_user_meet_invites = async (req, res) => {
	...
	if (user) {
		const meets = await Meet.find({ _id: { $in: user.pendingMeets } })
			.populate("createdby")
			.populate("participants")
			.catch((err) => console.log(err));
		// console.log(meets)
	...
	if (user) {
		...
		if (meet) {
			const newNotification = await Notification.create({
				sender: userid,
				reciever: meet.createdby.id,
				type: "invite",
				message: `${user.name} has accepted your meet invite`,
			}).catch((err) => console.log(err));
			await User.findByIdAndUpdate(userid, {
				$pull: { pendingMeets: meetId, notifications: notification?.id },
				$push: { meets: meetId },
			}).catch((err) => console.log(err));
			console.log(user.pendingMeets);
			await User.findByIdAndUpdate(meet.createdby.id, {
...
exports.clear_notification = async (req, res) => {
	...
	if (user) {
		...
		console.log(user.notifications);
		await Notification.findByIdAndDelete(notifId).catch((err) =>
			console.log(err)
		);
